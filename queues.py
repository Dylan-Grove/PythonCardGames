...
def main():
    ...
    # print(bq) and print(str(bq)) are not the same thing
    # The method isEmpty() does return the expected result; the
    # queue is not empty as we enqueued 'bob'
    ...
    # The queue does store the items in the expected order. The isFull() 
    # and size() methods also give the expected results. 
    ...
    # Test removing an element from full queue
    item = bq.dequeue()
    print(repr(bq))
    print(item,"was first in the bounded queue:", bq)
    print("There are", bq.size(), "elements in my bounded queue.")
    
    # Yes, an item can be removed from a full bounded queue. The 
    # dequeue methos returns the expected item; first in first out. 
    # The contents of bq are updated.
    ...
    # Capacity is the total number of items that are allowed to be in 
    # the queue and size is the number of items that are in the queue at
    # a certain time. 
    ...
    #  print(bq.__capacity) gives an AttributeError; we cannot access private attributes 
    # directly outside a Class definition
